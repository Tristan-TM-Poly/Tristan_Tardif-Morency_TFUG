...
productization layer, Knowledge Respect layer, and Human/AI feedback layer.
This is a review assistant, not an autonomous publisher. It does not merge PRs,
deploy, create payment links, upload Drive files, contact third parties, or
promote scientific/revenue/cultural claims.
"""
...
WORKFLOWS = [
    ...
    ".github/workflows/feedback_loop_governance.yml",
]
...
def check_feedback_docs() -> None:
    loop = ROOT / "docs" / "feedback" / "HUMAN_AI_FEEDBACK_LOOP.md"
    text = loop.read_text(encoding="utf-8").lower()
    for token in ["feedback improves direction", "evidence promotes status", "collect", "classify", "route", "patch", "review"]:
        if token not in text:
...
def main() -> int:
    ...
    run_script("tools/feedback_loop_ci.py")
